...
const Area = ({data, currentR, sendSubmission, showError}) => {
    ...
    const handleClick = (event) => {
        if (!currentR || isNaN(currentR) || currentR <= 0 || currentR > 3) {
    ...
    return (
        <svg id="interactiveArea" onClick={handleClick} width="350" height="350" style={{border: '1px solid black'}}>
            ...
            <text x="158" y="92.5" textAnchor="end" alignmentBaseline="middle"
                  fontSize="9px">{isValidR ? R / 2 : 'R/2'}</text>
            <line x1="170" y1="257.5" x2="180" y2="257.5" stroke="black"/>
            <text x="158" y="257.5" textAnchor="end" alignmentBaseline="middle"
                  fontSize="9px">{isValidR ? -R / 2 : '-R/2'}</text>
            <line x1="170" y1="340" x2="180" y2="340" stroke="black"/>
            <text x="158" y="340" textAnchor="end" alignmentBaseline="middle"
                  fontSize="9px">{isValidR ? -R : '-R'}</text>
